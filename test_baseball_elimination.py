...
def assert_eliminated(division, team):
    ...
    try:
        ...
        print(
            "Failure in linear programming is eliminated: "
            + team.name
    ...
    try:
        ...
        print(
            "Failure in network flows is eliminated: "
            + team.name
            + " should be eliminated."
        )
...
def test_teams2():
    ...
    for (ID, team) in division.teams.items():
        if (
            team.name == "NewYork"
            or team.name == "Baltimore"
            or team.name == "Boston"
            or team.name == "Toronto"
        ):
...
def test_teams7():
    ...
    for (ID, team) in division.teams.items():
        if team.name == "Ireland" or team.name == "China":
            ...
            team.name == "U.S.A."
            or team.name == "England"
            or team.name == "France"
            or team.name == "Germany"
            or team.name == "Belgium"
        ):
            assert_not_eliminated(division, team)
